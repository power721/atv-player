...
class FeiniuController:
    ...
    def _map_feiniu_items(self, payload: dict) -> list[VodItem]:
    ...
    def load_items(
        ...
        items = self._map_feiniu_items(payload)
    ...
    def search_items(self, keyword: str, page: int) -> tuple[list[VodItem], int]:
        payload = self._api_client.search_feiniu_items(keyword, page=page)
        items = self._map_feiniu_items(payload)
        total_raw = payload.get("total")
        if total_raw is not None:
            total = int(total_raw)
    ...
    def resolve_playlist_item(self, item: PlayItem) -> VodItem | None:
        if not item.vod_id:
            return None
        try:
            payload = self._api_client.get_feiniu_detail(item.vod_id)
