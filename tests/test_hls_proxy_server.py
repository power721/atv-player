...
"""
    ...
    first_status, first_headers, first_body = server.handle_request("GET", f"/m3u?token={token}")
    second_status, second_headers, second_body = server.handle_request("GET", f"/m3u?token={token}")

    expected_body = (
        "#EXTM3U\n#EXTINF:5.0,\nhttp://127.0.0.1:2323/seg?token="
        f"{token}&i=0\n"
    ).encode("utf-8")
