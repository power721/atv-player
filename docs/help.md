...
- 登录成功后，管理员需要选择要使用的全局 `vod token`；普通用户固定使用 `u-{用户名}` 作为用户级 `vod token`
- 应用不会保存密码
...
2. 输入后端地址、用户名和密码。
3. 若为管理员，选择要使用的全局 `vod token`；普通用户会自动使用 `u-{用户名}`。
4. 登录成功后进入主窗口。
...
- **界面主题**：选择浅色、深色或跟随系统
- **VOD Token**：仅管理员可见的独立设置区域，可切换当前使用的全局资源配置 token；普通用户始终使用 `u-{用户名}` 用户级 token
